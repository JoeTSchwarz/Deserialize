...
public class ODBObjectView {
  ...
  */
  public ODBObjectView(String fileName) throws Exception {
    try {
  ...
  @exception Exception thrown by JAVA (e.g. IOException, etc.)
  */
   public ODBObjectView(byte[] bb) throws Exception {
   view(bb);
  }
  /**
  ...
  */
  ...
  // private constructor
  private ODBObjectView(byte[] bb, int q, int ref, boolean array) throws Exception {
    this.ref = ref;
      ...
      if (t.indexOf("[") < 0) {
        switch (t) {
          ...
          default:
            if (bb[p] == (byte)0x73 && bb[p+1] == (byte)0x72) { // nested Object
              ODBObjectView ov = new ODBObjectView(bb, p+2, ref, array);
              if (ov.getSize() > 0) {
        ...
        default:
          ...
          if (bb[p] == (byte)0x73 && bb[p+1] == (byte)0x72) {
            ODBObjectView ov = new ODBObjectView(bb, p+2, ref, true);
            String cls = ov.getClassName();
            pFields.put(cls, ov.getOTFields());
            pNames.put(cls, ov.getFieldNames());
