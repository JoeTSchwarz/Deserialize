...
public class SerObjectView {
  ...
  */
    ...
    if (clsName.startsWith("java")) {
      while (true) {
        if (bb[p] == (byte)0x78) {
          if (bb[p+1] == (byte)0x70) break;
            ...
            //p += 2;
            //clsName = getString();
            p += (((int)(bb[p+2]&0xFF)<<8)|(int)(bb[p+3]&0xFF))+2;
          } else ++p;
        } else ++p;
      }
      // +2: over 0x7870      
      ...
      for (int i = 0; i < nFields && p < bb.length; ++i) {
        ...
        if (bb[p] == (byte)0x74) {
          ...
          if (e > 0) {
            if (tmp.indexOf("java") >= 0 && !tmp.endsWith("List") &&
                tmp.indexOf("String") < 0 && !tmp.endsWith("BigInteger"))
                throw new Exception(tmp+" API is not supported");
            ...
            if (bb[p] == (byte)0x73 && bb[p+1] == (byte)0x72) { // nested Object
              ...
              if (ov.getSize() > 0) {
                ...
                if (!t.startsWith("java")) tFields.put(n, t);
                     ...
                     tFields.put(n, t.substring(t.lastIndexOf(".")+1)+"<"+ov.getArrayType()+">");
                else tFields.put(n, t.substring(t.lastIndexOf(".")+1)); 
  ...
  private void loadAPI( ) throws Exception {
    if (clsName.endsWith("Integer")) { // BigInteger
      while (p < bb.length) if (bb[p] == (byte)0x78) {
        if (bb[p+1] == (byte)0x70) { p += 2; break; }
        ++p;
      } else ++p;
      le = getInt();
      byte[] val = new byte[le];
      System.arraycopy(bb, p, val, 0, le);
      object = new java.math.BigInteger(val);
      return;
    }
    int cap = getInt();
