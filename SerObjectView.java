  ...
  */
          ...
          default:
            if (bb[p] == (byte)0x73 && bb[p+1] == (byte)0x72) { // nested Object
              ...
              if (ov.getSize() > 0) {
                ...
                t = ov.getClassName();
                if (t.startsWith("java")) t = t.substring(t.lastIndexOf(".")+1);                
                tFields.put(n, t+"<"+ov.getArrayType()+">");
  ...
  private void loadAPI() throws Exception {
    ...
    if (bb[p] == (byte)0x73 && bb[p+1] == (byte)0x72) {
      p += 2;
      int l = getShort();
      String s = new String(bb, p, l, charset); // get Object type
      if (s.startsWith("java")) listType = s.substring(s.lastIndexOf(".")+1);
      p += l; // move to EndDataBlock/TC_NULL
    ...
    while (p < bb.length) {
      ...
      if (by == (byte)0x74) {
        ++p; // String
        int l = getShort();
        list.add(new String(bb, p, l, charset));
        p += l;
