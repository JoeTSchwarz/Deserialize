...
<br>Note:
<br>- Supported API: String, BigInteger, BigDecimal, Integer, Long, Short, Double, Float, Character, ArrayList
<br>- Array of Java APIs and ArrayList are NOT supported (except the supported API)
<br>- Arrays of custom serialized object or array of more than 3 dimensions are NOT supported.
...
public class SerObjectView {
  ...
  */
  ...
  // private constructor
  private SerObjectView(byte[] bb, int q, int ref) throws Exception {
    this.p = q;
    this.bb = bb;
    this.ref = ref;
    view(true);
  ...
  private void getValues( ) throws Exception {
    ...
    for (int i = 0; i < mx && p < bb.length; ++i) {
      ...
      if ((bb[p] == TC_OBJECT || bb[p] == TC_ARRAY) && bb[p] == TC_REFERENCE) ++p;
      if ((p+2) < bb.length && bb[p] == TC_REFERENCE && bb[p+1] == x00) {
        p += 3;
        int x = getShort()+cnt;
        if (nRef.contains(x)) {
          vFields.put(n, vRef.get(x));
          continue;
        }
      }
      if (t.indexOf("[") < 0) {
          ...
          default:
            if (bb[p] == TC_OBJECT && bb[p+1] == TC_CLASSDESC) { // nested Object
              if (t.endsWith("Integer") || t.endsWith("Decimal") || t.endsWith("List") || 
                ...
                SerObjectView ov = new SerObjectView(bb, p+2, ref);
  ...
  private Object execMethod(String mName) throws Exception {
    Method method = SerObjectView.class.getDeclaredMethod(mName, new Class[] {});
        ...
        if (bb[p+4] == TC_ARRAY && bb[p+5] == TC_REFERENCE) {
          ...
          if (bb[p] == TC_OBJECT) {
            if (bb[p+1] == TC_REFERENCE) {
              ...
              else { // not BigInteger or BigDecimal
